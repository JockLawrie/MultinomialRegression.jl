...
function construct_alligator_dict(data, xnames, ylevels)
    result = fill("", 219, 5)
    rownum = 0
    for profile = 1:16
    ...
    Dict(colnames[j] => view(result, :, j) for j = 1:length(colnames))
end
...
data    = construct_alligator_dict(rawdata, xnames, ylevels)

intercept = fill(1.0, length(data["Size"]))
male      = [x == "m"        ? 1.0 : 0.0 for x in data["Gender"]]  # levels = ["f", "m"]
large     = [x == ">2.3"     ? 1.0 : 0.0 for x in data["Size"]]    # levels = ["<2.3", ">2.3"]
george    = [x == "george"   ? 1.0 : 0.0 for x in data["Lake"]]    # levels = ["hancock", "george", "oklawaha", "trafford"]
oklawaha  = [x == "oklawaha" ? 1.0 : 0.0 for x in data["Lake"]]
trafford  = [x == "trafford" ? 1.0 : 0.0 for x in data["Lake"]]
...
X = hcat(intercept, george, oklawaha, trafford, large, male)
