...
# Unregularized fit
fit0   = fit(y, X)  # fit0 == (params=xxx, stderror=xxx)
B0     = fit0.params
...
@test isapprox(B0, B2; atol=1e-10)  # Reproducible result

# L1 regularized fit
fitL1  = fit(y, X, L1(0.5))
BL1    = fitL1.params
...
fitBox = fit(y, X, BoxRegularizer(-5.0, 5.0))
Bbox   = fitBox.params
iris.p = [predict(B, X[i, :]) for i = 1:nrow(iris)]
pmean  = mean([iris.p[i][y[i]] for i = 1:nrow(iris)])
